...
<template>
    <GuestLayout>
        ...
        <div
            v-if="status"
            class="mb-4 rounded-lg bg-green-500/20 p-3 text-sm font-medium text-green-200 backdrop-blur-sm border border-green-500/30"
        >
            {{ status }}
        </div>

        <!-- Deskripsi -->
        <div class="mb-6 text-sm text-white/70 leading-relaxed bg-white/5 p-4 rounded-lg border border-white/10">
            <div class="flex items-start gap-3">
                <svg class="h-5 w-5 text-white/50 flex-shrink-0 mt-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
                </svg>
                <span>Lupa password? Tenang saja. Masukkan alamat email Anda dan kami akan mengirimkan link reset password untuk membuat password baru.</span>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-5">
            ...
            <div class="mt-4 text-center">
                <Link
                    :href="route('login')"
                    class="inline-flex items-center gap-2 text-sm text-white/60 hover:text-white transition font-medium"
                >
